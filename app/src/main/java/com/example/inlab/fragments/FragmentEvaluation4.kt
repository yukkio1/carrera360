...
import com.example.inlab.databinding.FragmentEvaluation4Binding
...
class Evaluation4Fragment : Fragment(R.layout.fragment_evaluation4) {
    private var _binding: FragmentEvaluation4Binding? = null
    private val binding get() = _binding!!
    private val usuarioViewModel: UsuarioViewModel by activityViewModels()
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        _binding = FragmentEvaluation4Binding.bind(view)

        Log.d("Evaluation", "Fragmento Evaluation4Fragment iniciado correctamente.")
    ...
    private fun setupNavigation() {
        requireActivity().findViewById<BottomNavigationView>(R.id.bottomNavigationView)
            .setOnItemSelectedListener { item ->
                when (item.itemId) {
                    R.id.home_fragment -> {
                        findNavController().navigate(Evaluation4FragmentDirections.actionEvaluation4ToHome())
                        true
                    }
                    R.id.learn_fragment -> {
                        findNavController().navigate(Evaluation4FragmentDirections.actionEvaluation4ToLearn())
                        true
                    }
                    R.id.profile_fragment -> {
                        findNavController().navigate(Evaluation4FragmentDirections.actionEvaluation4ToProfile())
                        true
                    }
                    else -> false
                }
    ...
    private fun setupButtons() {
        binding.btnBack.setOnClickListener {
            findNavController().navigate(Evaluation4FragmentDirections.actionEvaluation4ToModule4())
        }

        binding.btnSubmitEvaluation4.setOnClickListener {
            evaluarRespuesta()
        }
    }

    private fun evaluarRespuesta() {
        val selectedOptionId = binding.radioGroupEvaluation4.checkedRadioButtonId
        ...
        val action = Evaluation4FragmentDirections.actionEvaluation4ToResult4(respuesta)
