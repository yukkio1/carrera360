...
import com.example.inlab.databinding.FragmentEvaluation6Binding
import com.example.inlab.viewmodel.UsuarioViewModel
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class Evaluation6Fragment : Fragment(R.layout.fragment_evaluation6) {
    private var _binding: FragmentEvaluation6Binding? = null
    private val binding get() = _binding!!
    private val usuarioViewModel: UsuarioViewModel by activityViewModels()

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        _binding = FragmentEvaluation6Binding.bind(view)

        Log.d("Evaluation", "Fragmento Evaluation6 iniciado correctamente.")
    ...
    private fun setupButtons() {
        binding.btnBack.setOnClickListener {
            findNavController().navigate(Evaluation6FragmentDirections.actionEvaluation6ToResult6("respuesta_seleccionada"))
        }

        binding.btnSubmitEvaluation6.setOnClickListener {
            evaluarRespuesta()
        }
    }
    ...
    private fun evaluarRespuesta() {
        ...
        val action = Evaluation6FragmentDirections.actionEvaluation6ToResult6(combinedOptions)
        findNavController().navigate(action)
    }

    private fun registrarEvaluacionRealizada() {
        val idUsuario = usuarioViewModel.idUsuario.value
        val idEvaluacion = 6 // Evaluación correspondiente al módulo 6
